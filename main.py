...
使用方式：修改下方 MODE 变量即可切换颜色识别算法。
  MODE = "hsv"           → 带先验知识无监督版（白色饱和度+色相聚类）
  MODE = "knn"           → 有监督KNN版（需CSV训练数据集）
  MODE = "unsupervised"  → 纯无监督K-means版
...
from color_recognition import (
    color_classification_hsv,
    color_classification_knn,
    color_classification_unsupervised,
)
...
# 颜色识别算法选择 —— 修改这里即可切换版本
# ============================================================
MODE = "hsv"  # 可选: "hsv" | "knn" | "unsupervised"
...
def outline_frame(cam, height, width):
    ...
    cube_frame = frame[y - 240:y + 240, x - 240:x + 240]
...
def recognize_cube(cam, myserial, color_func):
    """
    自动采集魔方6个面并识别颜色。
    ...
    color_func : callable
        颜色识别函数，接口 color_classification_xxx() → ndarray(54,) str
...
# ============================================================
# 机械臂姿态映射（exec_0to5 ~ exec_4to5）
# ============================================================

def exec_0to5():
    for i in range(6):
        if cur[i] == 0: cur[i] = 5
        elif cur[i] == 1: cur[i] = 4
        elif cur[i] == 2: cur[i] = 2
        elif cur[i] == 3: cur[i] = 3
        elif cur[i] == 4: cur[i] = 0
        elif cur[i] == 5: cur[i] = 1


def exec_1to5():
    for i in range(6):
        if cur[i] == 0: cur[i] = 4
        elif cur[i] == 1: cur[i] = 5
        elif cur[i] == 2: cur[i] = 2
        elif cur[i] == 3: cur[i] = 3
        elif cur[i] == 4: cur[i] = 1
        elif cur[i] == 5: cur[i] = 0


def exec_2to5():
    for i in range(6):
        if cur[i] == 0: cur[i] = 3
        elif cur[i] == 1: cur[i] = 2
        elif cur[i] == 2: cur[i] = 5
        elif cur[i] == 3: cur[i] = 4
        elif cur[i] == 4: cur[i] = 0
        elif cur[i] == 5: cur[i] = 1
...
def exec_3to5():
    for i in range(6):
        if cur[i] == 0: cur[i] = 2
        elif cur[i] == 1: cur[i] = 3
        elif cur[i] == 2: cur[i] = 4
        elif cur[i] == 3: cur[i] = 5
        elif cur[i] == 4: cur[i] = 0
        elif cur[i] == 5: cur[i] = 1
...
def exec_4to5():
    for i in range(6):
        if cur[i] == 0: cur[i] = 1
        elif cur[i] == 1: cur[i] = 0
        elif cur[i] == 2: cur[i] = 2
        elif cur[i] == 3: cur[i] = 3
        elif cur[i] == 4: cur[i] = 5
        elif cur[i] == 5: cur[i] = 4
...
def step_run(step):
    ...
    for j in range(len(step)):
        ...
        if j + 1 < len(step):
            ...
            next_step = 0
        if step[j] == 1:
            if temp_state == "locked_rotor": return "locked_rotor"
            res += "R"
        elif step[j] == 2:
            if temp_state == "locked_rotor": return "locked_rotor"
            res += "A"
...
# 颜色识别函数映射表
COLOR_FUNCTIONS = {
    "hsv": color_classification_hsv,
    "knn": color_classification_knn,
    "unsupervised": color_classification_unsupervised,
}
...
def main(mode=None):
    ...
    ----------
    mode : str or None
        "hsv" | "knn" | "unsupervised"，为 None 时使用文件顶部的 MODE 常量。
    """
    if mode is None:
        mode = MODE

    if mode not in COLOR_FUNCTIONS:
        print("错误：MODE 必须为 %s，当前值: %s" % (list(COLOR_FUNCTIONS.keys()), mode))
        sys.exit(1)

    color_func = COLOR_FUNCTIONS[mode]
    print("使用颜色识别模式: %s → %s" % (mode, color_func.__name__))

    cam = cv2.VideoCapture(0)
    if not cam.isOpened():
        raise Exception("Failed to open the camera.")

    solution = recognize_cube(cam, myserial, color_func)
    print(solution)
